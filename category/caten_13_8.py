...
#---
from type.ALL_decades import *   #ALL_Decade
from type.ALL_Years import *     #ALL_Year
...
def D_Y_C(goverment , pop_start , type):
    ...
    old_lenth = len(goverment.keys()) * len(pop_start.keys()) 
    #mha = islahvillage
    ...
    for pop in pop_st.keys():
        ...
        if pop != '' :
            ...
            for gov in goverment.keys():
                ...
                #pywikibot.output(category)
                opopo = {'category': category , 'gov': gov}
                if check:
                    #if type == 'ydc':
                        #if tat in Main_Type['d'] or tat in Main_Type['y'] or tat in Main_Type['c']:
                            #soooort.append(category)
                    #else:
                    if tat in Main_Type[type]:
                            soooort.append(opopo)
                else:
                    soooort.append(opopo)
            #log(gov , 'done')
        #else:
            #pywikibot.output( 'gov "%s" already in donelist.' % gov )
        if len(soooort) > 0:
            translations[pop] = soooort
    #---
    num = 0
    pop_num = 0
    newlenth = 0
    #row = [x for x in translations.keys()]
    for pop3 in translations.keys():
        newlenth += len(translations[pop3])
    #---
    pywikibot.output( '<<lightyellow>>>> old_lenth: %d , newlenth: %d ' % ( old_lenth , newlenth) )
    for pop2 in translations.keys():
        pop_num += 1
        poptable = translations[pop2]
        lenbob = len(translations[pop2])
        if lenbob > 0:
            pywikibot.output( '<<lightyellow>>>> %d/%d >> %s len:(%d)<< <<' % ( pop_num , len(translations.keys()), pop , lenbob) )
            for opopo in poptable:
                category = opopo['category']
                gov2 = opopo['gov']
                num += 1
                pywikibot.output( '<<lightblue>> a %d/%d/%d cat:"%s":' % ( pop_num , num ,newlenth,  category) )
                #pywikibot.output( '<<lightblue>> a %s: %d/%d/%d cat:"%s":' % (pop_st[pop]['ar'] , num , pop_num , newlenth , category) )
                work(category , pop_st[pop2] , goverment[gov2] )
