...
def logme(wditem, newlabel,oldlabel):
  form = '\n%s\tLar\t"%s"'
  form2 = '%s update: %s'
  with codecs.open("c30/fix.log.csv", "a", encoding="utf-8") as logfile:
    try:   
        logfile.write(form % (wditem.title(), newlabel) )
    except :
        pass
        print(" Error writing to logfile on: [%s]" %  newlabel)
    verbose = False#True    #now I want to see what!   
    logfile.close()
    if verbose:
