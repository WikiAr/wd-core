...
def makeComma(lang ,contry , place):
    ...
    return coma
#---
from ydn.Contries import ContriesTable
from ydn.places import placesTable
from ydn.correct import CorrectList
from ydn.replacement import replacement
from ydn.replacement2 import replacement2
#---
...
def AddDesc(data , item , Q_P17 , p31code , CountryName):
    ...
    if (Q_P17 in ContriesTable) and (p31code in placesTable):
        for lang in ContriesTable[Q_P17]:
            if lang in placesTable[p31code]:
                des = placesTable[p31code][lang] + ' ' + makeComma(lang ,contry , p31code) + ' ' + ContriesTable[Q_P17][lang]
                if lang in ['ar' , 'en', 'nl']:
    ...
    for lang2 in translations.keys():
        if lang2 not in item.labels:
            ...
            pywikibot.output('*Add "%s" descriptions: %s' % (lang2 , desc) )
    return data
#---
...
def Claims_for_item(item , geonames ,p31code ,  page ):
    ...
    #---
    #if summary != '':
    pywikibot.output('*descriptions : \n%s' % str(data["descriptions"]) )
    pywikibot.output('*descriptions : \n%s' % str(data["labels"]) )
    pywikibot.output('*descriptions : \n%s' % str(data["claims"].keys()) )
    sdo = False
    if addclaims :
