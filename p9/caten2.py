...
def D_Y_C(goverment , pop_start , type):
    ...
    for gov in goverment.keys():
        ...
        if gov != ''  and goverment[gov]['Q'] != '' :
            ...
            for pop in pop_st.keys():
                ...
                if pop != '' and pop_st[pop]['Q']  != '' :
                    ...
                    opopo = {'category': category , 'pop': pop}
                    if check:
        ...
        if len(soooort) > 0:
            translations[gov] = soooort
    #---
    num = 0
    gov_num = 0
    newlenth = 0
    #row = [x for x in translations.keys()]
    for gov3 in translations.keys():
        newlenth += len(translations[gov3])
        #pywikibot.output( '<<lightyellow>> gov3:' + gov3) 
        #pywikibot.output( translations[gov3] ) 
    #---
    pywikibot.output( '<<lightyellow>>>> old_lenth: %d , newlenth: %d ' % ( old_lenth , newlenth) )
    for gov2 in translations.keys():
        gov_num += 1
        gov_table = translations[gov2]
        lenbob = len(translations[gov2])
        if lenbob > 0:
            pywikibot.output( '<<lightyellow>>>> %d/%d >> gov2 : %s len:(%d)<< <<' % ( gov_num , len(translations.keys()), gov2 , lenbob) )
            for opopo in gov_table:
                category = opopo['category']
                pop2 = opopo['pop']
                num += 1
                pywikibot.output( '<<lightblue>> a %d/%d/%d cat:"%s":' % ( gov_num , num ,newlenth,  category) )
                #pywikibot.output( '<<lightblue>> a %s: %d/%d/%d cat:"%s":' % (pop_st[pop]['ar'] , num , gov_num , newlenth , category) )
