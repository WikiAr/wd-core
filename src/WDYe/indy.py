...
def main():
    ...
    query = (
        'SELECT ?item WHERE {  ?item rdfs:label ?cid22. FILTER((LANG(?cid22)) = "en").  ?item rdfs:label ?itemabel filter (lang(?itemabel) = "ar") BIND( REGEX(STR(?itemabel), "^((٠|١|٢|٣|٤|٥|٦|٧|٨|٩)*)$") AS ?regexresult ) .  FILTER( ?regexresult = true ) . } LIMIT '
        + str(max)
        + ""
    )
