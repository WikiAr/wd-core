...
from wd_core.wd_api import wd_desc
# wd_desc.wwdesc(new_desc_data, qid, i, fixlang, ask="", tage='')
# wd_desc.work_api_desc(new_desc_data, qid)
...
def wwdesc(newdesc, qid, i, fixlang, ask: str = "", tage: str = ""):
    ...
    Args:
        new_desc_data (dict): A dictionary containing new descriptions keyed by language codes.
    ...
    Returns:
        tuple: A tuple containing a boolean indicating success and the updated new_desc_data
            dictionary.
    """
    ...
    if fixlang:
        for ii in fixlang:
            if ii not in newdesc.keys():
                ...
                logger.info(f'remove "{ii}" from fixlang because it\'s not in new_desc_data')
        # ---
        fixed = ",".join(fixlang)
        # ---
    ...
    # ---
    logger.info(
        f'* wd_desc.py wwdesc "{qid}" try number:"{i}", len new_desc_data:"{len(newdesc)}", len queries_list:"{len(queries_list)}"'
    )
    # ---
    ...
    if "printdisc" in sys.argv:
        logger.info(data3)
    # ---
    skipp = WdApiBot.New_Mult_Des_2(qid, data3, summary, "", return_result=True, ask=ask, tage=tage)
    # ---
    if not skipp:
...
def work_api_desc(newdesc, qid, fixlang=None) -> None:
    ...
    if len(langes) == 1:
        ...
        WdApiBot.des_api(qid, onedesc, lang)
        return
    # ---
