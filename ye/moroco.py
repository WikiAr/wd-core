...
targetparamnew = {'إجمالي السكان' : 'P1082',
                  'عدد الأسر' : 'P1538',
                  'الذكور' : 'P1540',
                  'الإناث' : 'P1539',
                  }
...
def run_with_wikidata(params, titl1, tryy, item):
      ...
      return properties, v
      if there == True:
         pywikibot.output('**no param : %s' % targetparamnew )       # الاسم العلمي غير موجود في القالب
...
def PPP(params):	
  ...
  for pa in params:
    ...
    pa = re.sub('\s*\=\s*', '=', pa)
    pa = re.sub('\s*\"\"\s*', '', pa)
    pa = re.sub('\s*(نسمة|أسرة)\s*', '', pa)
    pa = re.sub('\<small\>.*?\<\/small\>', '', pa)
    pa = re.sub('\<ref name\=\"pp\"\>.*?\<\/ref\>', '', pa)
    #pa = pa.replace('<small>(2004)</small><ref name="pp">الإحصاء العام للسكان والسكنى لسنة 2004. [[الجريدة الرسمية للمملكة المغربية|الجريدة الرسمية]]. المملكة المغربية.</ref>', '')
    c.append(pa)
    #pywikibot.output(pa)
  return c
